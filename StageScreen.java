...
* 28122016: Updated total score position for visibility
*/
...
public class StageScreen implements Screen{
	...
	public void render(float delta) {
		...
		*/
		...
		game.CD.stringDrawing(game.sr,game.w*0.5f-5*(w+gap),game.h-h-gap,w,h,pw,gap,"CATEGORIES",game.CC.getCharColor());
				...
				game.CD.stringDrawing(game.sr,gap,y1/3+2*(2*gap+pw)-2*h-gap,game.w-2*gap,2*pw,0,0,"_",game.CC.bitColor);//hor 3
				...
				game.CD.stringDrawing(game.sr,2*gap+pw,y1/3+2*(3*gap+pw)-gap-h,w*0.5f,h*0.5f,pw*0.5f,gap,"SPEED:"+(int)game.pref.getFloat("4maxsp")/200+"X",game.CC.getCharColor());
				game.CD.stringDrawing(game.sr,2*gap+pw,y1/3+2*(3*gap+pw)-2*h-gap,w*0.5f,h*0.5f,pw*0.5f,gap,"BEST:"+game.pref.getFloat("4best"),game.CC.getCharColor());
				
				game.CD.stringDrawing(game.sr,game.w*0.5f+pw+gap,y1/3+2*(3*gap+pw)-gap-h,w*0.5f,h*0.5f,pw*0.5f,gap,"SPEED:"+(int)game.pref.getFloat("8maxsp")/200+"X",game.CC.getCharColor());
				game.CD.stringDrawing(game.sr,game.w*0.5f+pw+gap,y1/3+2*(3*gap+pw)-2*h-gap,w*0.5f,h*0.5f,pw*0.5f,gap,"BEST:"+game.pref.getFloat("8best"),game.CC.getCharColor());
	...
	private boolean getStage(int x, int y) {		
        // To convert camera's y axis which starts from top corner to screens y axis which starts from bottom
		y=game.h-y;	
		if(y>h+3*gap+4*pw && y<y1/3+2*(2*gap+pw)-2*h-gap)
		{
			if(x<game.w*0.5f)
			{
				game.st=16;
		...
		else if (y>y1/3+2*(2*gap+2*pw)-2*h-gap && y<2*y1/3+2*(2*gap+pw-h)-2*h-gap)
		{
			if(x<game.w*0.5f)
			{
				game.st=4;
